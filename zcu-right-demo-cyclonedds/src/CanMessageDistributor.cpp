...
void CanMessageDistributor::addParseMap(const std::unordered_map<uint32_t, std::function<void(const uint8_t * data)>>& parseMap) {
...
void CanMessageDistributor::onFlushCan(const std::unordered_map<uint32_t, udp_can_packet_t *>& packetMap) {
...
void CanMessageDistributor::receive_loop() {
    ...
    memset(canPacket, 0, sizeof(struct udp_can_packet_s));
