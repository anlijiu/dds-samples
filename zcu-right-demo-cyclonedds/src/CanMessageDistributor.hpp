...
using ParseMapFromCan = std::unordered_map<uint32_t, std::vector<std::function<void(const uint8_t * data)>>>;

public:
    CanMessageDistributor();

    void addParseMap(const std::unordered_map<uint32_t, std::function<void(const uint8_t * data)>>&);
    /**
    * @brief: 开启接收线程和发送线程
    */
    void start();
    ...
    */
    void onFlushCan(const std::unordered_map<uint32_t, udp_can_packet_t *>& packetMap);
private:
    void receive_loop();
