...
public:
    ...
    SendCanCache(std::function<void(const std::unordered_map<uint32_t, udp_can_packet_t *>&)> onFlush) : mOnFlush(onFlush) {
...
private:
    ...
    std::function<void(const std::unordered_map<uint32_t, udp_can_packet_t *>&)> mOnFlush;
